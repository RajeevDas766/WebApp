...
const { getWishlist,addToWishlist,removeToWishlist} = require("./../handlers/wishlist-handler");
...
// const { getCartItems } = require("../handlers/shopping-cart-handler");
...
router.delete("/wishlists/:id", async(req,res)=>{
    ...
    const userId = req.user.id;
    const productId = req.params.id;
    await removeToWishlist(userId, productId);
    res.send({message: "ok"});
});
